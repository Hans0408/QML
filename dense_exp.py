"""
Dynamic MPS -> VQC Bars-and-Stripes pipeline.

The script first trains an SO-parameterized MPS, then passes its trained
angle tensors directly into a dynamically constructed VQC.
...
def xor_grouped_rotation_pairs(matrix_dim: int):
    """Return rotation planes grouped by Gray-ordered XOR masks."""
...
def _left_apply_plane_rotation(
    ...
    """Left-multiply a matrix by the two-level rotation R_ij(theta)."""
...
def _rotation_matrix(
    ...
    pairs = factor_order if reverse_factor_order else reversed(factor_order)
...
def rotation_matrix_nd(n: int, thetas: torch.Tensor) -> torch.Tensor:
    return _rotation_matrix(
        n,
        thetas,
        reverse_factor_order=False,
    )


def rotation_matrix_nd_reversed(
    n: int,
    thetas: torch.Tensor,
) -> torch.Tensor:
    return _rotation_matrix(
...
class MPS1(nn.Module):
    """SO-parameterized MPS classifier."""
    ...
    def __init__(
        ...
        bulk_angle_count = number_of_so_angles(
            bond_dim * physical_dim
        )
    ...
    def middle_tensors(self):
        ...
        matrices = rotation_matrix_nd_reversed(
            full_dim,
            self.middle_thetas,
        )
    ...
    def last_tensor(self):
        ...
        matrix = rotation_matrix_nd_reversed(
            full_dim,
            self.last_thetas,
        )
...
class MPSInitializedDynamicVQC(nn.Module):
    """Dynamic VQC initialized directly from MPS parameters."""
    ...
    def encode_image(self, image):
        ...
        for site, wire in enumerate(
            self.geometry.image_wires
        ):
            qml.RY(
    ...
    def apply_extra_gates(self, extra_thetas):
        ...
        for layer in range(EXTRA_GATE_LAYERS):
            for gate_index, pair in enumerate(
                self.extra_gate_pairs
            ):
...
def build_vqc_from_initialization(
    ...
    if initialization == "pretrained":
        first_thetas = (
            mps_model.first_thetas.detach().clone()
        )
        middle_thetas = (
            mps_model.middle_thetas.detach().clone()
        )
        last_thetas = (
            mps_model.last_thetas.detach().clone()
        )
...
def run_one_seed(seed: int):
    ...
    for initialization in (
        ...
        vqc_model.set_evaluation_shots(
            EVALUATION_SHOTS
        )
...
def print_multi_seed_summary(all_results):
    ...
    for initialization in (
        ...
        initial_mean, initial_std = mean_and_std(
            initial_values
        )
        final_mean, final_std = mean_and_std(
            final_values
        )
